package by.domos.Kostkuj.server.trests;

import by.domos.Kostkuj.connect.MySQL.CraftCoin.MCraftCoins;
...
import by.domos.Kostkuj.connect.email.BanEmail;
import by.domos.Kostkuj.server.chat.BanManager.SendBanMessage;
import by.domos.Kostkuj.server.chat.SendSystem;
import by.domos.Kostkuj.server.player.ipmanager.IpHasher;
import by.domos.Kostkuj.server.BayPass;
...
public class Trest {
    ...
    public Trest(CommandSender sr, String[] args) {
        ...
        Timestamp expir = by.domos.Kostkuj.server.time.Time.getTimeSec(ftime);
        String cc = "";
        if (faction == 1 || faction == 2) {
            ...
            int idrstu = mt.setTrest(faction, admin, fbanneduuid, 0, desc + cc, by.domos.Kostkuj.server.time.Time.getTimeSec(0), expir, 0, frule);
            KickFromServer(idrstu, user, action, sr.getName(), desc + cc, by.domos.Kostkuj.server.time.Time.getTimeSec(0), expir, frule);
            ...
            int idrstu = mt.setTrest(faction, admin, fbanneduuid, fipid, desc + cc, by.domos.Kostkuj.server.time.Time.getTimeSec(0), expir, 0, frule);
            KickIpFromServer(idrstu, user, action, sr.getName(), fipid, desc + cc, by.domos.Kostkuj.server.time.Time.getTimeSec(0), expir, frule);
            if (userid != 0) {
            ...
            int idrsu = mt.setTrest(faction, admin, fbanneduuid, 0, desc, by.domos.Kostkuj.server.time.Time.getTimeSec(0), by.domos.Kostkuj.server.time.Time.getTimeSec(ftime), 0, frule);
            SendBanMessage sbm = new SendBanMessage();
            Time tims = new Time();
            sbm.sendBanMessage(user, sr.getName(), action, tims.getTimeFromTimeStamp(by.domos.Kostkuj.server.time.Time.getTimeSec(0)), tims.getTimeFromTimeStamp(by.domos.Kostkuj.server.time.Time.getTimeSec(ftime)), idrsu, desc, rule);
        } else {
            ...
            int idrstu = mt.setTrest(faction, admin, fbanneduuid, 0, desc, by.domos.Kostkuj.server.time.Time.getTimeSec(0), by.domos.Kostkuj.server.time.Time.getTimeSec(0), 1, frule);
            KickFromServer(idrstu, user, action, sr.getName(), desc, by.domos.Kostkuj.server.time.Time.getTimeDay(0), by.domos.Kostkuj.server.time.Time.getTimeDay(0), frule);
        }
