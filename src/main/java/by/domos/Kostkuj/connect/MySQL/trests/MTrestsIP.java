...
public class MTrestsIP {
    ...
    public void setIPFirstLogin(String uuid, String haship) {
        if (m.isConected()) {
            ...
            try {
                ps.setString(1, uuid);
                ps.setString(2, haship);
                ps.setTimestamp(3, by.domos.Kostkuj.server.time.Time.getTimeDay(0));
                ps.executeUpdate();
                ps.close();
            } catch (SQLException e) {
                e.printStackTrace();
    ...
    public void updateIPLogin(String uuid, String haship) {
        if (m.isConected()) {
            ...
            try {
                ps.setTimestamp(1, by.domos.Kostkuj.server.time.Time.getTimeDay(0));
                ps.setString(2, uuid);
            ...
            try {
                ...
                if (rowAffected == 1) {
                    ...
                    if (rs.next()) {
                        ...
                        ps2.setTimestamp(3, by.domos.Kostkuj.server.time.Time.getTimeDay(0));
                        ps2.executeUpdate();
                    }
                }
            } catch (SQLException e) {
